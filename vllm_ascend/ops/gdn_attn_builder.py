...
def _build_non_spec_chunked_prefill_metadata(
    ...
    num_decodes = sum(
        1
        for seq_start, seq_end in zip(cu_seqlens_host, cu_seqlens_host[1:])
...
class AscendGDNAttentionMetadataBuilder(GDNAttentionMetadataBuilder):
    ...
    def _reset_spec_decode_graph_inputs(self, graph_batch_size: int) -> None:
        ...
        Full-graph capture always builds speculative GDN metadata when spec
        decoding is enabled. A DP rank or mixed batch can later replay that
        graph without any runtime spec requests. Refresh every persistent spec
        input consumed by the captured conv1d/recurrent tasks so capture-time
        values cannot advance GDN state.
        """
    ...
    def _fold_spec_sized_prefill_chunks_into_spec(
        ...
        """Restore the stateful GDN contract for spec-sized prompt chunks.
        ...
        decode graph. Fold the row into the spec metadata with all tokens
        accepted, which advances its conv/recurrent state over the whole chunk
        exactly like the prefill path would. Rows without prior state (first
        chunks) stay on the prefill path.
        """
        is_prefilling = common_attn_metadata.is_prefilling
        seq_lens_cpu = common_attn_metadata.seq_lens_cpu_upper_bound
