...
def get_kv_cache_spec(vllm_config: VllmConfig) -> dict[str, KVCacheSpec]:
    ...
    if mamba_specs:
        common_page_size = max(
            spec.page_size_bytes for spec in (*kv_cache_spec.values(), *mamba_specs.values())
        )
...
def _allocate_kv_cache(
    ...
    has_attention = any(
        isinstance(spec, AttentionSpec) for spec in layer_kv_cache_spec.values()
    )
        ...
        for layer_name in group.layer_names:
            ...
            if isinstance(kv_cache_spec, MambaSpec):
                ...
                if mamba_cache[0].shape[0] < kv_cache_config.num_blocks:
                    raise ValueError(
                        f"Mamba cache for {layer_name} has fewer blocks than KVCacheManager."
                    )
